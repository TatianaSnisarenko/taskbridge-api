...
export const tasksSchemas = {
  CreateTaskDraftRequest: {
    ...
    properties: {
      ...
      timezone_preference: { type: 'string', example: 'Europe/Kyiv' },
      ...
      deliverables: {
        ...
        example: ['Production-ready API endpoints', 'Tests', 'Updated Swagger docs'],
      },
      requirements: {
        type: 'array',
        maxItems: 30,
        items: { type: 'string', minLength: 3, maxLength: 2000 },
        example: ['Node.js', 'Prisma', 'PostgreSQL', 'REST API design skills'],
      },
      nice_to_have: {
        type: 'array',
        maxItems: 30,
        items: { type: 'string', minLength: 3, maxLength: 2000 },
        example: ['Query optimization experience', 'OpenAPI schema design'],
      },
    },
  },
  ...
  UpdateTaskDraftRequest: {
    ...
    properties: {
      ...
      deliverables: {
        ...
        maxItems: 30,
        items: { type: 'string', minLength: 3, maxLength: 2000 },
        example: ['API implementation', 'Tests', 'Documentation updates'],
      },
      requirements: {
        type: 'array',
        maxItems: 30,
        items: { type: 'string', minLength: 3, maxLength: 2000 },
        example: ['Strong Node.js backend skills', 'SQL query tuning'],
      },
      nice_to_have: {
        type: 'array',
        maxItems: 30,
  ...
  ReportTaskRequest: {
    ...
    properties: {
      ...
      comment: {
        type: 'string',
        maxLength: 1000,
        example: 'This task contains spam-like promotional content.',
      },
    },
  },
  ReportTaskResponse: {
  ...
  TaskReportQueueItem: {
    ...
    properties: {
      report_id: { type: 'string', format: 'uuid' },
      target_type: { type: 'string', enum: ['task'] },
  ...
  GetTaskReportsResponse: {
    ...
    properties: {
      items: {
        type: 'array',
        items: { $ref: '#/components/schemas/TaskReportQueueItem' },
      },
  ...
  ResolveTaskReportRequest: {
    ...
    properties: {
      ...
      note: {
        ...
        minLength: 3,
        maxLength: 2000,
        example: 'Confirmed policy violation after moderation review.',
      },
    },
  },
  ResolveTaskReportResponse: {
    type: 'object',
    properties: {
      report_id: { type: 'string', format: 'uuid' },
  ...
  TaskDetailsResponse: {
    ...
    properties: {
      ...
      requirements: {
        ...
        maxItems: 30,
        items: { type: 'string', minLength: 3, maxLength: 2000 },
        example: ['Strong Node.js', 'Prisma', 'SQL', 'Test coverage discipline'],
      },
      nice_to_have: {
        type: 'array',
        maxItems: 30,
        items: { type: 'string', minLength: 3, maxLength: 2000 },
        example: ['OpenAPI-first development experience'],
      },
      created_at: { type: 'string', format: 'date-time' },
      published_at: { type: 'string', format: 'date-time', nullable: true },
